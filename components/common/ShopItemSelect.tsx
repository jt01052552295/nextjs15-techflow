...
import { fetchAllShopItemAction } from '@/actions/shop/item/list';
...
const ShopItemSelect: React.FC<SelectProps> = ({
  name,
  control,
  defaultValue = '',
  disabled = false,
  ...
  const { data: list = [], isLoading } = useQuery({
    queryKey: ['all-shop-items', 'active'],
    queryFn: async () => {
      return await fetchAllShopItemAction();
    },
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.shop-item-select-container')) {
        setIsOpen(false);
      }
    };
  ...
  return (
    <div className={`mb-2 ${className}`}>
      ...
      <Controller
        ...
        render={({ field }) => {
          return (
            <div className="shop-item-select-container position-relative">
              {/* 선택된 사용자 표시 또는 입력 필드 */}
              <div
