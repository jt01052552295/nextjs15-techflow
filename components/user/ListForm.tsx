...
import { useUserInfinite } from '@/hooks/react-query/useUser';
...
import { listUpdateAction } from '@/actions/user/list/update';
...
type Props = { baseParams: UserBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<UserBaseParams>(baseParams);
  const [, setForm] = useState<UserBaseParams>(baseParams);
  ...
  const applyParams = (next: UserBaseParams) => {
  ...
  const handleApply = (next: UserBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useUserInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IUserListRow[] = useMemo(() => {
  ...
  const handleCheckAll = (checked: boolean) => {
    setCheckAll(checked);
    if (checked) {
      setSelectedUids(items.map((item) => item.id));
    } else {
      setSelectedUids([]);
    }
  };
  ...
  const handleFieldSave = async (
    ...
    const res = await listUpdateAction({ id, [field]: newValue });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('user.create', locale);
  ...
  return (
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.user.profile')}</th>
              <th className="text-center">{t('columns.user.id')}</th>
              <th className="text-center">{t('columns.user.name')}</th>
              <th className="text-center">{t('columns.user.email')}</th>
              <th className="text-center">{t('columns.user.createdAt')}</th>
              <th className="text-center">{t('columns.user.updatedAt')}</th>
              <th className="text-center">{t('columns.user.accounts')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: IUserListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
                  setSelectedRow={setSelectedRow}
                  isChecked={selectedUids.includes(row.id)}
                  onCheck={handleCheck}
      ...
      <DeleteConfirmModal
        ...
        ids={modalType === 'bulk' ? selectedUids : []}
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['user', params], (oldData: any) => {
            ...
            return {
              ...
              pages: oldData.pages.map((page: any) => ({
                ...
                items: page.items.filter(
                  (item: any) => !deletedUids.includes(item.id),
                ),
