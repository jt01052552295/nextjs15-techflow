...
import { DEFAULTS, type UserBaseParams } from '@/types/user/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: UserBaseParams; // 현재 URL에서 온 값
  onApply: (next: UserBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<UserBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: UserBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    ...
    const type =
      value.dateType === 'createdAt'
        ? t('columns.user.createdAt')
        : value.dateType === 'updatedAt'
          ? t('columns.user.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.user.createdAt'),
      updatedAt: t('columns.user.updatedAt'),
      idx: t('columns.user.idx'),
      name: t('columns.user.name'),
      email: t('columns.user.email'),
    };
    const by = map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.user.idx');
  ...
  return (
    <div className="mb-3">
      ...
      <div
        ...
        tabIndex={-1}
        id="userFilters"
        aria-labelledby="userFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="userFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.user.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
          ...
          {/* isVisible */}
          <div className="mb-3">
            <label className="form-label">{t('columns.user.isVisible')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.user.idx')}</option>
        ...
        <div className="offcanvas-body">
          <div className="row row-cols-2 g-2">
            <div className="col">
              ...
              >
                <option value="createdAt">{t('columns.user.createdAt')}</option>
                <option value="updatedAt">{t('columns.user.updatedAt')}</option>
                <option value="idx">{t('columns.user.idx')}</option>
                <option value="name">{t('columns.user.name')}</option>
                <option value="email">{t('columns.user.email')}</option>
              </select>
