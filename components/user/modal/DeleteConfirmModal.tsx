...
import { deleteAction } from '@/actions/user/delete';
...
const DeleteConfirmModal = ({ row, ids, onDeleted }: Props) => {
  const { t } = useLanguage();

  const handleDelete = async () => {
    const deletedUids: string[] = row ? [row.id] : (ids ?? []);
    if (deletedUids.length === 0) return;

    const formData = row ? { id: row.id } : { ids };
        ...
        <div className="modal-content">
          ...
          <div className="modal-body">
            {row ? (
              ...
              <p>{t('common.confirm_delete_multi', { count: ids?.length })}</p>
            )}
          </div>
