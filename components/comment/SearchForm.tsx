...
import { DEFAULTS, type CommentBaseParams } from '@/types/comment/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: CommentBaseParams; // 현재 URL에서 온 값
  onApply: (next: CommentBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<CommentBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: CommentBaseParams) => {
    ...
    const type =
      value.dateType === 'createdAt'
        ...
        : value.dateType === 'updatedAt'
          ? t('columns.bbsComment.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.bbsComment.createdAt'),
      likeCount: t('columns.bbsComment.likeCount'),
      replyCount: t('columns.bbsComment.replyCount'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ??
      t('columns.bbsComment.createdAt');
  ...
  return (
    <div className="mb-3">
        ...
        <div className="col-12 col-md-6">
          <div className="d-flex flex-wrap gap-2 h-100 justify-content-md-end align-items-md-center">
            ...
            <button
              ...
              data-bs-target="#commentFilters"
              aria-controls="commentFilters"
            >
              {t('common.filter')}
            </button>
      ...
      <div
        ...
        tabIndex={-1}
        id="commentFilters"
        aria-labelledby="commentFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="commentFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
