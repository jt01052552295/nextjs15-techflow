...
import { useBadgeInfinite } from '@/hooks/react-query/useBadge';
import ListRowSkeleton from './ListRowSkeleton';
import ListRow from './ListRow';
import type { IBadge, IBadgeListRow, ListEditCell } from '@/types/badge';
...
import {
  ...
  type BadgeBaseParams,
} from '@/types/badge/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/badge/list/update';
...
type Props = { baseParams: BadgeBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<BadgeBaseParams>(baseParams);
  const [, setForm] = useState<BadgeBaseParams>(baseParams);
  ...
  const applyParams = (next: BadgeBaseParams) => {
  ...
  const handleApply = (next: BadgeBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useBadgeInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IBadgeListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IBadge | null>(null);
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['badge', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('badge.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.badge.uid')}</th>
              <th className="text-center">{t('columns.badge.bmType')}</th>
              <th className="text-center">{t('columns.badge.bmName')}</th>
              <th className="text-center">{t('columns.badge.createdAt')}</th>
              <th className="text-center">{t('columns.badge.updatedAt')}</th>
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['badge', params], (oldData: any) => {
