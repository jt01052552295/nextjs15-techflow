...
type Props = {
  row: IBadgeListRow;
  setSelectedRow: (row: IBadge) => void;
  isChecked: boolean;
...
const ListRow = ({
  ...
  const showUrl = getRouteUrl('badge.show', locale, { id: row.uid });
  const editUrl = getRouteUrl('badge.edit', locale, { id: row.uid });
      ...
      <td className="text-center">
        <EditableCell
          ...
          onSave={(newVal, onSuccess, onError) =>
            onFieldSave(row.uid, 'bmType', newVal, onSuccess, onError)
          }
        />
      </td>
      <td className="text-center">
        <EditableCell
          value={row.bmName}
          onSave={(newVal, onSuccess, onError) =>
            onFieldSave(row.uid, 'bmName', newVal, onSuccess, onError)
          }
        />
      </td>
      <td className="text-center">{row.createdAt}</td>
      <td className="text-center">{row.updatedAt}</td>
      <td className="text-center">{row._count?.UserBadge ?? 0}</td>
      <td className="text-center">
