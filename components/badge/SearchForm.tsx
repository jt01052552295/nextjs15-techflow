...
import { DEFAULTS, type BadgeBaseParams } from '@/types/badge/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: BadgeBaseParams; // 현재 URL에서 온 값
  onApply: (next: BadgeBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<BadgeBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: BadgeBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    ...
    const type =
      value.dateType === 'createdAt'
        ? t('columns.badge.createdAt')
        : value.dateType === 'updatedAt'
          ? t('columns.badge.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.badge.createdAt'),
      updatedAt: t('columns.badge.updatedAt'),
      idx: t('columns.badge.idx'),
      name: t('columns.badge.bmType'),
      email: t('columns.badge.bmName'),
    };
    const by = map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.badge.idx');
  ...
  return (
    <div className="mb-3">
        ...
        <div className="col-12 col-md-6">
          <div className="d-flex flex-wrap gap-2 h-100 justify-content-md-end align-items-md-center">
            ...
            <button
              ...
              data-bs-target="#badgeFilters"
              aria-controls="badgeFilters"
            >
              {t('common.filter')}
            </button>
      ...
      <div
        ...
        tabIndex={-1}
        id="badgeFilters"
        aria-labelledby="badgeFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="badgeFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.badge.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
          ...
          {/* isVisible */}
          <div className="mb-3">
            <label className="form-label">{t('columns.badge.isVisible')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.badge.idx')}</option>
                  <option value="bmType">{t('columns.badge.bmType')}</option>
                  <option value="bmName">{t('columns.badge.bmName')}</option>
                </select>
              </div>
              <div className="col">
          ...
          <div className="row row-cols-2 g-2">
            <div className="col">
              ...
              >
                ...
                <option value="idx">{t('columns.badge.idx')}</option>
                <option value="name">{t('columns.badge.bmType')}</option>
                <option value="email">{t('columns.badge.bmName')}</option>
              </select>
            </div>
            <div className="col">
              <div className="btn-group w-100">
