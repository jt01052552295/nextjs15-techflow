...
import { DEFAULTS, type BlogPostBaseParams } from '@/types/blog/post/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: BlogPostBaseParams; // 현재 URL에서 온 값
  onApply: (next: BlogPostBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<BlogPostBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: BlogPostBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    ...
    const type =
      value.dateType === 'createdAt'
        ? t('columns.blogPost.createdAt')
        : value.dateType === 'updatedAt'
          ? t('columns.blogPost.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.blogPost.createdAt'),
      updatedAt: t('columns.blogPost.updatedAt'),
      idx: t('columns.blogPost.idx'),
    ...
    <div className="mb-3">
      ...
      >
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.blogPost.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.blogPost.idx')}</option>
                  <option value="sortOrder">
                    {t('columns.blogPost.sortOrder')}
                  </option>
                </select>
              </div>
              <div className="col">
