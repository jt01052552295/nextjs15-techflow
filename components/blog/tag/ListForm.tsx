...
import { useBlogTagInfinite } from '@/hooks/react-query/blog/useTag';
import ListRowSkeleton from './ListRowSkeleton';
import ListRow from './ListRow';
import type { IBlogTag, IBlogTagListRow, ListEditCell } from '@/types/blog/tag';
...
import {
  ...
  type BlogTagBaseParams,
} from '@/types/blog/tag/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/blog/tag/list/update';
...
type Props = { baseParams: BlogTagBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<BlogTagBaseParams>(baseParams);
  const [, setForm] = useState<BlogTagBaseParams>(baseParams);
  ...
  const applyParams = (next: BlogTagBaseParams) => {
  ...
  const handleApply = (next: BlogTagBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useBlogTagInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IBlogTagListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IBlogTag | null>(null);
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['blogTag', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('blogTag.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.blogTag.uid')}</th>
              <th className="text-center">{t('columns.blogTag.slug')}</th>
              <th className="text-center">{t('columns.blogTag.name')}</th>
              <th className="text-center">{t('columns.blogTag.createdAt')}</th>
              <th className="text-center">{t('columns.blogTag.updatedAt')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: IBlogTagListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['blogTag', params], (oldData: any) => {
