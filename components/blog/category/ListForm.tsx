...
import { useBlogCategoryInfinite } from '@/hooks/react-query/blog/useCategory';
import ListRowSkeleton from './ListRowSkeleton';
import ListRow from './ListRow';
import type {
  IBlogCategory,
  IBlogCategoryListRow,
  ListEditCell,
...
} from '@/types/blog/category/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/blog/category/list/update';
...
type Props = { baseParams: BlogCategoryBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<BlogCategoryBaseParams>(baseParams);
  const [, setForm] = useState<BlogCategoryBaseParams>(baseParams);
  ...
  const applyParams = (next: BlogCategoryBaseParams) => {
  ...
  const handleApply = (next: BlogCategoryBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useBlogCategoryInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IBlogCategoryListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IBlogCategory | null>(null);
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['blogCategory', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('blogCategory.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.blogCategory.uid')}</th>
              <th className="text-center">{t('columns.blogCategory.code')}</th>
              <th className="text-center">{t('columns.blogCategory.name')}</th>
              <th className="text-center">
                {t('columns.blogCategory.createdAt')}
              </th>
              <th className="text-center">
                {t('columns.blogCategory.updatedAt')}
              </th>
          ...
          <tbody>
            {!isLoading &&
              items.map((row: IBlogCategoryListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['blogCategory', params], (oldData: any) => {
