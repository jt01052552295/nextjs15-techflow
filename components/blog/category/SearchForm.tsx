...
type Props = {
  value: BlogCategoryBaseParams; // 현재 URL에서 온 값
  onApply: (next: BlogCategoryBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<BlogCategoryBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: BlogCategoryBaseParams) => {
  ...
  const isVisibleBtnLabel =
    value.isVisible === undefined
      ? t('columns.blogCategory.isVisible')
      : value.isVisible
        ? t('common.visible')
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.blogCategory.createdAt'),
      updatedAt: t('columns.blogCategory.updatedAt'),
      idx: t('columns.blogCategory.idx'),
      name: t('columns.blogCategory.name'),
      code: t('columns.blogCategory.code'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.blogCategory.idx');
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.blogCategory.idx')}</option>
                  <option value="name">{t('columns.blogCategory.name')}</option>
                  <option value="code">{t('columns.blogCategory.code')}</option>
                </select>
              </div>
              <div className="col">
