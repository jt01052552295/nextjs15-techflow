...
type Props = {
  value: BlogPostCommentBaseParams; // 현재 URL에서 온 값
  onApply: (next: BlogPostCommentBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<BlogPostCommentBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: BlogPostCommentBaseParams) => {
  ...
  const isUseBtnLabel =
    value.isUse === undefined
      ? t('columns.blogPostComment.isUse')
      : value.isUse
        ? t('common.usage')
        : t('common.usageNone');
  ...
  const isVisibleBtnLabel =
    value.isVisible === undefined
      ? t('columns.blogPostComment.isVisible')
      : value.isVisible
        ? t('common.visible')
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.blogPostComment.createdAt'),
      updatedAt: t('columns.blogPostComment.updatedAt'),
      idx: t('columns.blogPostComment.idx'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.blogPostComment.idx');
          ...
          <div className="row row-cols-2 g-2">
            <div className="col">
              ...
              >
                ...
                <option value="idx">{t('columns.blogPostComment.idx')}</option>
              </select>
            </div>
            <div className="col">
              <div className="btn-group w-100">
