...
import { useBlogPostCommentInfinite } from '@/hooks/react-query/blog/useComment';
...
} from '@/types/blog/comment/search';
...
type Props = { baseParams: BlogPostCommentBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<BlogPostCommentBaseParams>(baseParams);
  const [, setForm] = useState<BlogPostCommentBaseParams>(baseParams);
  ...
  const applyParams = (next: BlogPostCommentBaseParams) => {
  ...
  const handleApply = (next: BlogPostCommentBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useBlogPostCommentInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IBlogPostCommentListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IBlogPostComment | null>(null);
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('blogComment.create', locale);
    ...
    <div className="position-relative">
      ...
      <div className="table-responsive">
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">
                {t('columns.blogPostComment.uid')}
              </th>
              <th className="text-center">
                {t('columns.blogPostComment.userId')}
              </th>
              <th className="text-center">
                {t('columns.blogPostComment.content')}
              </th>
              <th className="text-center">
                {t('columns.blogPostComment.status')}
              </th>
              <th className="text-center">
                {t('columns.blogPostComment.createdAt')}
              </th>
              <th className="text-center">
                {t('columns.blogPostComment.updatedAt')}
              </th>
          ...
          <tbody>
            {!isLoading &&
              items.map((row: IBlogPostCommentListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          ...
          queryClient.invalidateQueries({
            queryKey: ['blogPostComment', params],
          });
