...
import { DEFAULTS, type AddressBaseParams } from '@/types/address/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: AddressBaseParams; // 현재 URL에서 온 값
  onApply: (next: AddressBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<AddressBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: AddressBaseParams) => {
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      idx: t('columns.address.idx'),
      userId: t('columns.address.userId'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.address.idx');
  ...
  return (
    <div className="mb-3">
        ...
        <div className="col-12 col-md-6">
          <div className="d-flex flex-wrap gap-2 h-100 justify-content-md-end align-items-md-center">
            ...
            <button
              ...
              data-bs-target="#addressFilters"
              aria-controls="addressFilters"
            >
              {t('common.filter')}
            </button>
      ...
      <div
        ...
        tabIndex={-1}
        id="addressFilters"
        aria-labelledby="addressFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="addressFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
          ...
          <div className="mb-3">
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  <option value="idx">{t('columns.address.idx')}</option>
                  <option value="userId">{t('columns.address.userId')}</option>
                </select>
              </div>
              <div className="col">
