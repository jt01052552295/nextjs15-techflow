...
import { useBbsInfinite } from '@/hooks/react-query/useBbs';
...
import {
  ...
  type BbsBaseParams,
} from '@/types/bbs/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/bbs/list/update';
import { listSortAction } from '@/actions/bbs/list/sort';
...
type Props = { baseParams: BbsBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<BbsBaseParams>(baseParams);
  const [, setForm] = useState<BbsBaseParams>(baseParams);
  ...
  const applyParams = (next: BbsBaseParams) => {
  ...
  const handleApply = (next: BbsBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useBbsInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IBBSListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IBBSListRow | null>(null);
  ...
  const handleMove = async (direction: 'up' | 'down' | 'top' | 'bottom') => {
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['bbs', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('bbs.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.bbs.uid')}</th>
              <th className="text-center">{t('columns.bbs.bdTable')}</th>
              <th className="text-center">{t('columns.bbs.subject')}</th>
              <th className="text-center">{t('columns.bbs.name')}</th>
              <th className="text-center">{t('columns.bbs.createdAt')}</th>
              <th className="text-center">{t('columns.bbs.updatedAt')}</th>
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['bbs', params], (oldData: any) => {
