...
import { useCommentsInfinite } from '@/hooks/react-query/useBbs';
...
export default function CommentSection({ bdTable, pid }: Props) {
  ...
  const [commentToDelete, setCommentToDelete] = useState<IBBSCommentRow | null>(
    null,
  );
  ...
  // 댓글 목록 가공
  const comments: IBBSCommentRow[] = useMemo(() => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { content: string; parentIdx?: number | null }) =>
      createCommentAction({ bdTable, pid, ...data }),
    onSuccess: (updatedData, newComment) => {
      ...
      if (!newComment.parentIdx) {
        ...
        queryClient.setQueryData(bbsQK.comments(rootBase), (oldData: any) => {
        ...
        queryClient.setQueryData(bbsQK.comments(rootBase), (oldData: any) => {
          ...
          return {
            ...
            pages: oldData.pages.map((page: any) => ({
              ...page,
              items: page.items.map((comment: IBBSCommentRow) =>
      ...
      setCommentForm({ bdTable, content: '', parentIdx: null });
      setReplyToId(null);
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, content }: { id: string; content: string }) =>
      updateCommentAction({ pid, uid: id, content }),
  ...
  const deleteMutation = useMutation({
    mutationFn: (row: IBBSCommentRow) => deleteCommentAction(row),
        ...
        return {
          ...
          pages: oldData.pages.map((page: any) => ({
            ...
            items: page.items.filter(
              (comment: IBBSCommentRow) => comment.uid !== deletedComment.uid,
            ),
          })),
        };
      });
  ...
  const likeMutation = useMutation({
    mutationFn: (commentId: number) =>
      likeCommentAction({ bdTable, commentId }),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: bbsQK.comments(rootBase),
      });
  ...
  const handleReplyTo = (commentId: number) => {
    ...
    if (replyToId === commentId) {
      ...
      setCommentForm({ bdTable, content: '', parentIdx: commentId });
    }
  };
