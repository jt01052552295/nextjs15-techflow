...
import { useCompanyInfinite } from '@/hooks/react-query/useCompany';
import ListRowSkeleton from './ListRowSkeleton';
import ListRow from './ListRow';
import type { ICompany, ListEditCell } from '@/types/company';
import ScrollToTopButton from '../common/ScrollToTopButton';
import {
  DEFAULTS,
  isSameBaseParams,
  toSearchParamsFromBase,
  type CompanyBaseParams,
} from '@/types/company/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/company/list/update';
...
type Props = { baseParams: CompanyBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<CompanyBaseParams>(baseParams);
  const [, setForm] = useState<CompanyBaseParams>(baseParams);
  ...
  const applyParams = (next: CompanyBaseParams) => {
  ...
  const handleApply = (next: CompanyBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useCompanyInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: ICompany[] = useMemo(() => {
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('company.create', locale);
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.company.uid')}</th>
              <th className="text-center">{t('columns.company.userId')}</th>
              <th className="text-center">{t('columns.company.name')}</th>
              <th className="text-center">{t('columns.company.bizNo')}</th>
              <th className="text-center">{t('columns.company.isUse')}</th>
              <th className="text-center">{t('columns.company.isVisible')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: ICompany) => (
                <ListRow
                  key={row.idx}
                  row={row}
