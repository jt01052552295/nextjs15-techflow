...
import { DEFAULTS, type CompanyBaseParams } from '@/types/company/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: CompanyBaseParams; // 현재 URL에서 온 값
  onApply: (next: CompanyBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<CompanyBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: CompanyBaseParams) => {
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      idx: t('columns.company.idx'),
      userId: t('columns.company.userId'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.company.idx');
  ...
  return (
    <div className="mb-3">
      ...
      <div
        ...
        tabIndex={-1}
        id="companyFilters"
        aria-labelledby="companyFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="companyFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
          ...
          <div className="mb-3">
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  <option value="idx">{t('columns.company.idx')}</option>
                  <option value="userId">{t('columns.company.userId')}</option>
                </select>
              </div>
              <div className="col">
