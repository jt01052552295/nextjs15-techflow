...
import { DEFAULTS, type BoardBaseParams } from '@/types/board/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: BoardBaseParams; // 현재 URL에서 온 값
  onApply: (next: BoardBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<BoardBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: BoardBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    ...
    const type =
      value.dateType === 'createdAt'
        ? t('columns.board.createdAt')
        : value.dateType === 'updatedAt'
          ? t('columns.board.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.board.createdAt'),
      updatedAt: t('columns.board.updatedAt'),
      idx: t('columns.board.idx'),
      bdName: t('columns.board.bdName'),
      bdTable: t('columns.board.bdTable'),
      sortOrder: t('columns.board.sortOrder'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.board.sortOrder');
  ...
  return (
    <div className="mb-3">
        ...
        <div className="col-12 col-md-6">
          <div className="d-flex flex-wrap gap-2 h-100 justify-content-md-end align-items-md-center">
            ...
            <button
              ...
              data-bs-target="#boardFilters"
              aria-controls="boardFilters"
            >
              {t('common.filter')}
            </button>
      ...
      <div
        ...
        tabIndex={-1}
        id="boardFilters"
        aria-labelledby="boardFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="boardFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.board.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
          ...
          {/* isVisible */}
          <div className="mb-3">
            <label className="form-label">{t('columns.board.isVisible')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.board.idx')}</option>
                  <option value="bdTable">{t('columns.board.bdTable')}</option>
                  <option value="bdName">{t('columns.board.bdName')}</option>
                  <option value="sortOrder">
                    {t('columns.board.sortOrder')}
                  </option>
                </select>
              </div>
              <div className="col">
