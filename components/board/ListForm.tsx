...
import { useBoardInfinite } from '@/hooks/react-query/useBoard';
...
} from '@/types/board/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/board/list/update';
import { listSortAction } from '@/actions/board/list/sort';
...
type Props = { baseParams: BoardBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<BoardBaseParams>(baseParams);
  const [, setForm] = useState<BoardBaseParams>(baseParams);
  ...
  const applyParams = (next: BoardBaseParams) => {
  ...
  const handleApply = (next: BoardBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useBoardInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IBoardListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IBoardListRow | null>(null);
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('board.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.board.uid')}</th>
              <th className="text-center">{t('columns.board.bdTable')}</th>
              <th className="text-center">{t('columns.board.bdName')}</th>
              <th className="text-center">{t('columns.board.createdAt')}</th>
              <th className="text-center">{t('columns.board.updatedAt')}</th>
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['board', params], (oldData: any) => {
