...
import { DEFAULTS, type CategoryBaseParams } from '@/types/category/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: CategoryBaseParams; // 현재 URL에서 온 값
  onApply: (next: CategoryBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<CategoryBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: CategoryBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    ...
    const type =
      value.dateType === 'createdAt'
        ? t('columns.category.createdAt')
        : value.dateType === 'updatedAt'
          ? t('columns.category.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.category.createdAt'),
      updatedAt: t('columns.category.updatedAt'),
      idx: t('columns.category.idx'),
      name: t('columns.category.name'),
      code: t('columns.category.code'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.category.idx');
    ...
    <div className="mb-3">
      ...
      >
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.category.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.category.idx')}</option>
                  <option value="name">{t('columns.category.name')}</option>
                  <option value="code">{t('columns.category.code')}</option>
                </select>
              </div>
              <div className="col">
