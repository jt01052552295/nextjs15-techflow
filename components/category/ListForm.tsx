...
import { useCategoryInfinite } from '@/hooks/react-query/useCategory';
...
import {
  ...
  type CategoryBaseParams,
} from '@/types/category/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/category/list/update';
...
type Props = { baseParams: CategoryBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<CategoryBaseParams>(baseParams);
  const [, setForm] = useState<CategoryBaseParams>(baseParams);
  ...
  const applyParams = (next: CategoryBaseParams) => {
  ...
  const handleApply = (next: CategoryBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useCategoryInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: ICategoryListRow[] = useMemo(() => {
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['category', params] });
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.category.uid')}</th>
              <th className="text-center">{t('columns.category.code')}</th>
              <th className="text-center">{t('columns.category.name')}</th>
              <th className="text-center">{t('columns.category.createdAt')}</th>
              <th className="text-center">{t('columns.category.updatedAt')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: ICategoryListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['category', params], (oldData: any) => {
