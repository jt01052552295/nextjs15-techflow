...
import { DEFAULTS, type AgentBaseParams } from '@/types/agent/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: AgentBaseParams; // 현재 URL에서 온 값
  onApply: (next: AgentBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<AgentBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: AgentBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    if (!value.dateType && !value.startDate && !value.endDate)
      return t('columns.agent.createdAt');
    const type = t('columns.agent.createdAt');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.agent.createdAt'),
      idx: t('columns.agent.idx'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.agent.sortOrder');
              ...
              <div className="col">
                ...
                >
                  <option value="createdAt">
                    {t('columns.agent.createdAt')}
                  </option>
                  <option value="idx">{t('columns.agent.idx')}</option>
                </select>
              </div>
              <div className="col">
