...
import { DEFAULTS, type PopupBaseParams } from '@/types/popup/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: PopupBaseParams; // 현재 URL에서 온 값
  onApply: (next: PopupBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<PopupBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: PopupBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    ...
    const type =
      value.dateType === 'createdAt'
        ? t('columns.popup.createdAt')
        : value.dateType === 'updatedAt'
          ? t('columns.popup.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.popup.createdAt'),
      updatedAt: t('columns.popup.updatedAt'),
      idx: t('columns.popup.idx'),
    };
    const by = map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.popup.idx');
  ...
  return (
    <div className="mb-3">
        ...
        <div className="col-12 col-md-6">
          <div className="d-flex flex-wrap gap-2 h-100 justify-content-md-end align-items-md-center">
            ...
            <button
              ...
              data-bs-target="#popupFilters"
              aria-controls="popupFilters"
            >
              {t('common.filter')}
            </button>
      ...
      <div
        ...
        tabIndex={-1}
        id="popupFilters"
        aria-labelledby="popupFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="popupFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.popup.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
          ...
          {/* isVisible */}
          <div className="mb-3">
            <label className="form-label">{t('columns.popup.isVisible')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.popup.idx')}</option>
                </select>
              </div>
              <div className="col">
