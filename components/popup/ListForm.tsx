...
import { usePopupInfinite } from '@/hooks/react-query/usePopup';
import ListRowSkeleton from './ListRowSkeleton';
import ListRow from './ListRow';
import type { IPopup, IPopupListRow, ListEditCell } from '@/types/popup';
...
import {
  ...
  type PopupBaseParams,
} from '@/types/popup/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/popup/list/update';
...
type Props = { baseParams: PopupBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<PopupBaseParams>(baseParams);
  const [, setForm] = useState<PopupBaseParams>(baseParams);
  ...
  const applyParams = (next: PopupBaseParams) => {
  ...
  const handleApply = (next: PopupBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = usePopupInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IPopupListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IPopup | null>(null);
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['popup', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('popup.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.popup.uid')}</th>
              <th className="text-center">{t('columns.popup.title')}</th>
              <th className="text-center">{t('columns.popup.startTime')}</th>
              <th className="text-center">{t('columns.popup.endTime')}</th>
              <th className="text-center">{t('columns.popup.createdAt')}</th>
              <th className="text-center">{t('columns.popup.updatedAt')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: IPopupListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['popup', params], (oldData: any) => {
