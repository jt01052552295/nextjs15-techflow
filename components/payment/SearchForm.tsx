...
import { DEFAULTS, type PaymentBaseParams } from '@/types/payment/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: PaymentBaseParams; // 현재 URL에서 온 값
  onApply: (next: PaymentBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<PaymentBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: PaymentBaseParams) => {
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      idx: t('columns.payment.idx'),
      userId: t('columns.payment.userId'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.payment.idx');
  ...
  return (
    <div className="mb-3">
        ...
        <div className="col-12 col-md-6">
          <div className="d-flex flex-wrap gap-2 h-100 justify-content-md-end align-items-md-center">
            ...
            <button
              ...
              data-bs-target="#paymentFilters"
              aria-controls="paymentFilters"
            >
              {t('common.filter')}
            </button>
      ...
      <div
        ...
        tabIndex={-1}
        id="paymentFilters"
        aria-labelledby="paymentFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="paymentFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
          ...
          <div className="mb-3">
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  <option value="idx">{t('columns.payment.idx')}</option>
                  <option value="userId">{t('columns.payment.userId')}</option>
                </select>
              </div>
              <div className="col">
