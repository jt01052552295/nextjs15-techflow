...
import { usePaymentInfinite } from '@/hooks/react-query/usePayment';
...
import {
  ...
  type PaymentBaseParams,
} from '@/types/payment/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/payment/list/update';
...
type Props = { baseParams: PaymentBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<PaymentBaseParams>(baseParams);
  const [, setForm] = useState<PaymentBaseParams>(baseParams);
  ...
  const applyParams = (next: PaymentBaseParams) => {
  ...
  const handleApply = (next: PaymentBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = usePaymentInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IPayment[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IPayment | null>(null);
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['payment', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('payment.create', locale);
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.payment.uid')}</th>
              <th className="text-center">{t('columns.payment.userId')}</th>
              <th className="text-center">{t('columns.payment.name')}</th>
              <th className="text-center">{t('columns.payment.cardName')}</th>
              <th className="text-center">{t('columns.payment.isUse')}</th>
              <th className="text-center">{t('columns.payment.isVisible')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: IPayment) => (
                <ListRow
                  key={row.idx}
                  row={row}
