...
import { useAddressInfinite } from '@/hooks/react-query/useAddress';
import ListRowSkeleton from './ListRowSkeleton';
import ListRow from './ListRow';
import type { IAddress, ListEditCell } from '@/types/address';
import ScrollToTopButton from '@/components/common/ScrollToTopButton';
import {
  DEFAULTS,
  isSameBaseParams,
  toSearchParamsFromBase,
  type AddressBaseParams,
} from '@/types/address/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/address/list/update';
...
type Props = { baseParams: AddressBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<AddressBaseParams>(baseParams);
  const [, setForm] = useState<AddressBaseParams>(baseParams);
  ...
  const applyParams = (next: AddressBaseParams) => {
  ...
  const handleApply = (next: AddressBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useAddressInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IAddress[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IAddress | null>(null);
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['address', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('address.create', locale);
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.address.uid')}</th>
              <th className="text-center">{t('columns.address.userId')}</th>
              <th className="text-center">{t('columns.address.title')}</th>
              <th className="text-center">{t('columns.address.name')}</th>
              <th className="text-center">{t('columns.address.isUse')}</th>
              <th className="text-center">{t('columns.address.isVisible')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: IAddress) => (
                <ListRow
                  key={row.idx}
                  row={row}
