...
type Props = {
  value: FcmMessageBaseParams; // 현재 URL에서 온 값
  onApply: (next: FcmMessageBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<FcmMessageBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: FcmMessageBaseParams) => {
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      idx: t('columns.fcmMessage.idx'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.fcmMessage.idx');
          ...
          <div className="mb-3">
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  <option value="idx">{t('columns.fcmMessage.idx')}</option>
                </select>
              </div>
              <div className="col">
