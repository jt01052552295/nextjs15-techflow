...
import {
  ...
  type FcmAlarmBaseParams,
} from '@/types/fcm/alarm/search';
...
type Props = { baseParams: FcmAlarmBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<FcmAlarmBaseParams>(baseParams);
  const [, setForm] = useState<FcmAlarmBaseParams>(baseParams);
  ...
  const applyParams = (next: FcmAlarmBaseParams) => {
  ...
  const handleApply = (next: FcmAlarmBaseParams) => applyParams(next);
  ...
  const [selectedRow, setSelectedRow] = useState<IFcmAlarm | null>(null);
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('fcmAlarms.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.fcmAlarm.uid')}</th>
              <th className="text-center">{t('columns.fcmAlarm.userId')}</th>
              <th className="text-center">
                {t('columns.fcmAlarm.templateId')}
              </th>
              <th className="text-center">{t('columns.fcmAlarm.message')}</th>
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['fcmAlarm', params], (oldData: any) => {
          ...
          queryClient.invalidateQueries({ queryKey: ['fcmAlarm', params] });
