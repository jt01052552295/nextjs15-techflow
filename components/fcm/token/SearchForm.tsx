...
import { DEFAULTS, type FcmTokenBaseParams } from '@/types/fcm/token/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: FcmTokenBaseParams; // 현재 URL에서 온 값
  onApply: (next: FcmTokenBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<FcmTokenBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: FcmTokenBaseParams) => {
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      idx: t('columns.fcmToken.idx'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.fcmToken.idx');
          ...
          <div className="mb-3">
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  <option value="idx">{t('columns.fcmToken.idx')}</option>
                </select>
              </div>
              <div className="col">
