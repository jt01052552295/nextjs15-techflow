...
import {
  ...
  type BannerBaseParams,
} from '@/types/banner/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/banner/list/update';
import { listSortAction } from '@/actions/banner/list/sort';
...
type Props = { baseParams: BannerBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<BannerBaseParams>(baseParams);
  const [, setForm] = useState<BannerBaseParams>(baseParams);
  ...
  const applyParams = (next: BannerBaseParams) => {
  ...
  const handleApply = (next: BannerBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useBannerInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IBannerListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IBanner | null>(null);
  ...
  const handleMove = async (direction: 'up' | 'down' | 'top' | 'bottom') => {
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['banner', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('banner.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.banner.uid')}</th>
              <th className="text-center">{t('columns.banner.gubun')}</th>
              <th className="text-center">{t('columns.banner.title')}</th>
              <th className="text-center">{t('columns.banner.createdAt')}</th>
              <th className="text-center">{t('columns.banner.updatedAt')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: IBannerListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['banner', params], (oldData: any) => {
