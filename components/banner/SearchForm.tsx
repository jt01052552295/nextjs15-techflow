...
import { DEFAULTS, type BannerBaseParams } from '@/types/banner/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: BannerBaseParams; // 현재 URL에서 온 값
  onApply: (next: BannerBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<BannerBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: BannerBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    ...
    const type =
      value.dateType === 'createdAt'
        ? t('columns.banner.createdAt')
        : value.dateType === 'updatedAt'
          ? t('columns.banner.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.banner.createdAt'),
      updatedAt: t('columns.banner.updatedAt'),
      idx: t('columns.banner.idx'),
      gubun: t('columns.banner.gubun'),
      sortOrder: t('columns.banner.sortOrder'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.banner.sortOrder');
  ...
  return (
    <div className="mb-3">
        ...
        <div className="col-12 col-md-6">
          <div className="d-flex flex-wrap gap-2 h-100 justify-content-md-end align-items-md-center">
            ...
            <button
              ...
              data-bs-target="#bannerFilters"
              aria-controls="bannerFilters"
            >
              {t('common.filter')}
            </button>
      ...
      <div
        ...
        tabIndex={-1}
        id="bannerFilters"
        aria-labelledby="bannerFiltersLabel"
      >
        <div className="offcanvas-header">
          <h5 id="bannerFiltersLabel" className="offcanvas-title">
            {t('common.filter')}
          </h5>
          <button
            type="button"
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.banner.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.banner.idx')}</option>
                  <option value="gubun">{t('columns.banner.gubun')}</option>
                  <option value="sortOrder">
                    {t('columns.banner.sortOrder')}
                  </option>
                </select>
              </div>
              <div className="col">
