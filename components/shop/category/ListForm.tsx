...
import { useShopCategoryInfinite } from '@/hooks/react-query/shop/useCategory';
import ListRowSkeleton from './ListRowSkeleton';
import ListRow from './ListRow';
import type {
  IShopCategory,
  IShopCategoryListRow,
  ListEditCell,
} from '@/types/shop/category';
...
} from '@/types/shop/category/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/shop/category/list/update';
...
type Props = { baseParams: ShopCategoryBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<ShopCategoryBaseParams>(baseParams);
  const [, setForm] = useState<ShopCategoryBaseParams>(baseParams);
  ...
  const applyParams = (next: ShopCategoryBaseParams) => {
  ...
  const handleApply = (next: ShopCategoryBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useShopCategoryInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IShopCategoryListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IShopCategory | null>(null);
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['shopCategory', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('shopCategory.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.shopCategory.uid')}</th>
              <th className="text-center">{t('columns.shopCategory.code')}</th>
              <th className="text-center">{t('columns.shopCategory.name')}</th>
              <th className="text-center">
                {t('columns.shopCategory.createdAt')}
              </th>
              <th className="text-center">
                {t('columns.shopCategory.updatedAt')}
              </th>
          ...
          <tbody>
            {!isLoading &&
              items.map((row: IShopCategoryListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['shopCategory', params], (oldData: any) => {
