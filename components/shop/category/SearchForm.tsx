...
type Props = {
  value: ShopCategoryBaseParams; // 현재 URL에서 온 값
  onApply: (next: ShopCategoryBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<ShopCategoryBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: ShopCategoryBaseParams) => {
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.shopCategory.createdAt'),
      updatedAt: t('columns.shopCategory.updatedAt'),
      idx: t('columns.shopCategory.idx'),
      name: t('columns.shopCategory.name'),
      code: t('columns.shopCategory.code'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.shopCategory.idx');
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.shopCategory.idx')}</option>
                  <option value="name">{t('columns.shopCategory.name')}</option>
                  <option value="code">{t('columns.shopCategory.code')}</option>
                </select>
              </div>
              <div className="col">
