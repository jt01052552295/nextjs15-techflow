...
type Props = {
  value: ShopReviewBaseParams; // 현재 URL에서 온 값
  onApply: (next: ShopReviewBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<ShopReviewBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: ShopReviewBaseParams) => {
  ...
  const isVisibleBtnLabel =
    value.isVisible === undefined
      ? t('columns.shopReview.isVisible')
      : value.isVisible
        ? t('common.visible')
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.shopReview.createdAt'),
      updatedAt: t('columns.shopReview.updatedAt'),
      idx: t('columns.shopReview.idx'),
      name: t('columns.shopReview.name'),
      code: t('columns.shopReview.code'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.shopReview.idx');
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.shopReview.idx')}</option>
                  <option value="name">{t('columns.shopReview.name')}</option>
                  <option value="code">{t('columns.shopReview.code')}</option>
                </select>
              </div>
              <div className="col">
