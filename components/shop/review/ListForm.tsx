...
import { useShopReviewInfinite } from '@/hooks/react-query/shop/useReview';
...
} from '@/types/shop/review/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/shop/review/list/update';
...
type Props = { baseParams: ShopReviewBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<ShopReviewBaseParams>(baseParams);
  const [, setForm] = useState<ShopReviewBaseParams>(baseParams);
  ...
  const applyParams = (next: ShopReviewBaseParams) => {
  ...
  const handleApply = (next: ShopReviewBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useShopReviewInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IShopReviewListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IShopReview | null>(null);
  ...
  const handleFieldSave = async (
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['shopReview', params] });
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.shopReview.uid')}</th>
              <th className="text-center">{t('columns.shopReview.userId')}</th>
              <th className="text-center">{t('columns.shopReview.subject')}</th>
              <th className="text-center">
                {t('columns.shopReview.createdAt')}
              </th>
              <th className="text-center">
                {t('columns.shopReview.updatedAt')}
              </th>
          ...
          <tbody>
            {!isLoading &&
              items.map((row: IShopReviewListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['shopReview', params], (oldData: any) => {
