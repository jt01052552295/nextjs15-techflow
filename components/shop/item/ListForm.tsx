...
import { useShopItemInfinite } from '@/hooks/react-query/shop/useItem';
...
import {
  ...
  type ShopItemBaseParams,
} from '@/types/shop/item/search';
import SearchForm from './SearchForm';
import { listUpdateAction } from '@/actions/shop/item/list/update';
import { listSortAction } from '@/actions/shop/item/list/sort';
...
type Props = { baseParams: ShopItemBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<ShopItemBaseParams>(baseParams);
  const [, setForm] = useState<ShopItemBaseParams>(baseParams);
  ...
  const applyParams = (next: ShopItemBaseParams) => {
  ...
  const handleApply = (next: ShopItemBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useShopItemInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IShopItemListRow[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IShopItem | null>(null);
  ...
  const handleMove = async (direction: 'up' | 'down' | 'top' | 'bottom') => {
    ...
    if (res.status === 'success') {
      queryClient.invalidateQueries({ queryKey: ['shopItem', params] });
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('shopItem.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.shopItem.uid')}</th>
              <th className="text-center">{t('columns.shopItem.code')}</th>
              <th className="text-center">{t('columns.shopItem.name')}</th>
              <th className="text-center">{t('columns.shopItem.createdAt')}</th>
              <th className="text-center">{t('columns.shopItem.updatedAt')}</th>
          ...
          <tbody>
            {!isLoading &&
              items.map((row: IShopItemListRow) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['shopItem', params], (oldData: any) => {
