...
type Props = {
  row: IShopItemListRow;
  setSelectedRow: (row: IShopItem) => void;
  isChecked: boolean;
...
const ListRow = ({
  ...
  const showUrl = getRouteUrl('shopItem.show', locale, { id: row.uid });
  const editUrl = getRouteUrl('shopItem.edit', locale, { id: row.uid });
  ...
  return (
    <tr key={row.idx}>
      ...
      <td className="text-center">
        <span className="badge text-bg-secondary">{row.code}</span>
      </td>
      <td className="text-center">
        <EditableCell
          value={row.name}
      ...
      <td className="text-center">
        {row._count?.ShopItemFile ?? 0} {row._count?.ShopItemOption ?? 0}{' '}
        {row._count?.ShopItemSupply ?? 0}
      </td>
      <td className="text-center">
