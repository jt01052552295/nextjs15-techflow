...
import { DEFAULTS, type ShopItemBaseParams } from '@/types/shop/item/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: ShopItemBaseParams; // 현재 URL에서 온 값
  onApply: (next: ShopItemBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<ShopItemBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: ShopItemBaseParams) => {
  ...
  const dateBtnLabel = (() => {
    ...
    const type =
      value.dateType === 'createdAt'
        ? t('columns.shopItem.createdAt')
        : value.dateType === 'updatedAt'
          ? t('columns.shopItem.updatedAt')
          : t('common.period');
    const range =
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.shopItem.createdAt'),
      updatedAt: t('columns.shopItem.updatedAt'),
      idx: t('columns.shopItem.idx'),
      name: t('columns.shopItem.name'),
      email: t('columns.shopItem.email'),
      sortOrder: t('columns.shopItem.sortOrder'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.shopItem.sortOrder');
    ...
    <div className="mb-3">
      ...
      >
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.shopItem.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.shopItem.idx')}</option>
                  <option value="name">{t('columns.shopItem.name')}</option>
                  <option value="sortOrder">
                    {t('columns.shopItem.sortOrder')}
                  </option>
                </select>
              </div>
              <div className="col">
