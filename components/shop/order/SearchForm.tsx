...
import { DEFAULTS, type ShopOrderBaseParams } from '@/types/shop/order/search';
import { useLanguage } from '@/components/context/LanguageContext';

type Props = {
  value: ShopOrderBaseParams; // 현재 URL에서 온 값
  onApply: (next: ShopOrderBaseParams) => void; // 확인/검색 버튼 클릭 시만 호출
  onReset: () => void; // 초기화
  loading?: boolean;
};
...
export default function SearchForm({
  ...
  const [f, setF] = useState<ShopOrderBaseParams>(value);
  ...
  // 기간 정규화(시작>종료면 자동 스왑)
  const normalizePeriod = (next: ShopOrderBaseParams) => {
  ...
  const sortBtnLabel = (() => {
    const map: Record<string, string> = {
      createdAt: t('columns.shopOrder.createdAt'),
      idx: t('columns.shopOrder.idx'),
    };
    const by =
      map[value.sortBy ?? DEFAULTS.sortBy!] ?? t('columns.shopOrder.idx');
    ...
    <div className="mb-3">
      ...
      >
        ...
        <div className="offcanvas-body">
          ...
          {/* isUse */}
          <div className="mb-3">
            <label className="form-label">{t('columns.shopOrder.isUse')}</label>
            <div className="btn-group" role="group">
              <input
                type="radio"
                className="btn-check"
            ...
            <div className="row row-cols-2 g-2">
              <div className="col">
                ...
                >
                  ...
                  <option value="idx">{t('columns.shopOrder.idx')}</option>
                </select>
              </div>
              <div className="col">
