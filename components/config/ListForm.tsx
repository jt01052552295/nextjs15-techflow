...
import { useConfigInfinite } from '@/hooks/react-query/useConfig';
...
import { listUpdateAction } from '@/actions/config/list/update';
import { listSortAction } from '@/actions/config/list/sort';
...
type Props = { baseParams: ConfigBaseParams };
...
const ListForm = ({ baseParams }: Props) => {
  ...
  // 1) URL → baseParams
  const [params, setParams] = useState<ConfigBaseParams>(baseParams);
  const [, setForm] = useState<ConfigBaseParams>(baseParams);
  ...
  const applyParams = (next: ConfigBaseParams) => {
  ...
  const handleApply = (next: ConfigBaseParams) => applyParams(next);
  ...
  const {
    ...
    fetchNextPage,
    refetch,
  } = useConfigInfinite(params);

  // 평탄화 + 중복 제거(커서 경계에서 같은 uid가 들어오는 상황 방지)
  const items: IConfig[] = useMemo(() => {
  ...
  const [selectedRow, setSelectedRow] = useState<IConfig | null>(null);
  ...
  const handleCreate = () => {
    const baseUrl = getRouteUrl('config.create', locale);
    ...
    <div className="position-relative">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th className="text-center">{t('columns.config.uid')}</th>
              <th className="text-center">{t('columns.config.CNFname')}</th>
              <th className="text-center">{t('columns.config.CNFvalue')}</th>
              <th className="text-center">{t('columns.config.CNFvalue_en')}</th>
              <th className="text-center">{t('columns.config.CNFvalue_ja')}</th>
              <th className="text-center">{t('columns.config.CNFvalue_zh')}</th>
              <th className="text-center">{t('common.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {!isLoading &&
              items.map((row: IConfig) => (
                <ListRow
                  key={row.idx}
                  row={row}
      ...
      <DeleteConfirmModal
        ...
        onDeleted={(deletedUids) => {
          // 1. 먼저 캐시 직접 업데이트
          queryClient.setQueryData(['config', params], (oldData: any) => {
