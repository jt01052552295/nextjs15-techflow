...
import type { CreateType } from '@/actions/shop/category/create/schema';
import type { UpdateType } from '@/actions/shop/category/update/schema';
import { getChildCategories } from '@/lib/shop/category-utils';
...
export async function list(params: ListParams = {}): Promise<ListResult> {
  ...
  // ───────────────────────────────────
  let keysetWhere: Prisma.ShopCategoryWhereInput | undefined;
  if (cursor) {
  ...
  // orderBy
  // ───────────────────────────────────
  const orderBy: Prisma.ShopCategoryOrderByWithRelationInput[] = [
    { [sortBy]: order },
    { idx: order }, // tie-breaker도 동일 방향
  ];

  const whereForPage: Prisma.ShopCategoryWhereInput = keysetWhere
  ...
  const [totalAll, totalFiltered] = await Promise.all([
    prisma.shopCategory.count({ where: baseWhere }),
    prisma.shopCategory.count({ where: filteredWhere }),
  ]);
...
/** 보기 */
export async function show(uid: string): Promise<IShopCategory> {
  // 먼저 존재 확인 (선택)
  ...
  const rs = await prisma.shopCategory.findUnique({
    where: { uid },
  });
...
export async function update(input: UpdateType) {
  ...
  const exist = await prisma.shopCategory.findUnique({
    where: { uid, cid },
