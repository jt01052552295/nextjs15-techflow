...
export async function list(params: ListParams = {}): Promise<ListResult> {
  ...
  // where (검색/필터)
  // ───────────────────────────────────
  const baseWhere: Prisma.FcmTemplateWhereInput = {
    isUse: typeof isUse === 'boolean' ? isUse : true,
    isVisible: typeof isVisible === 'boolean' ? isVisible : true,
  };

  // 통합 검색(q)이 들어오면 name/email OR 매칭. 없으면 기존 name/email 개별 필드 사용
  const filteredWhere: Prisma.FcmTemplateWhereInput = q
    ? {
        AND: [
          baseWhere,
          {
            OR: [{ type: { contains: q } }, { activity: { contains: q } }],
          },
        ],
      }
    : {
        ...baseWhere,
        ...(type?.trim() ? { type: { contains: type.trim() } } : {}),
        ...(activity?.trim()
          ? { activity: { contains: activity.trim() } }
          : {}),
      };
  ...
  // ───────────────────────────────────
  let keysetWhere: Prisma.FcmTemplateWhereInput | undefined;
  if (cursor) {
  ...
  // orderBy
  // ───────────────────────────────────
  const orderBy: Prisma.FcmTemplateOrderByWithRelationInput[] = [
    { [sortBy]: order },
    { idx: order }, // tie-breaker도 동일 방향
  ];

  console.log(filteredWhere, q);

  const whereForPage: Prisma.FcmTemplateWhereInput = keysetWhere
