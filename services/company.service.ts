...
import type { CreateType } from '@/actions/company/create/schema';
import type { UpdateType } from '@/actions/company/update/schema';
...
export async function list(params: ListParams = {}): Promise<ListResult> {
  ...
  const filteredWhere: Prisma.CompanyWhereInput = q
    ? {
        ...baseWhere,
        OR: [
          { name: { contains: q } },
    ...
    : {
        ...
        ...(bizNo?.trim() ? { bizNo: { contains: bizNo.trim() } } : {}),
      };
  ...
  // ───────────────────────────────────
  let keysetWhere: Prisma.CompanyWhereInput | undefined;
  if (cursor) {
  ...
  const whereForPage: Prisma.CompanyWhereInput = keysetWhere
...
/** 보기 */
export async function show(uid: string): Promise<ICompany> {
  // 먼저 존재 확인 (선택)
  ...
  const rs = await prisma.company.findUnique({
    where: { uid },
    include: {
      // 필요 시 조건/정렬 조절
      user: true,
...
export async function create(input: CreateType) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const created = await tx.company.create(createData);

    // 관계 포함 최종 반환
    const withRelations = await tx.company.findUnique({
...
export async function remove(input: DeleteInput): Promise<DeleteResult> {
  ...
  if (uids && uids.length > 0) {
    return await prisma.$transaction(async (tx) => {
      if (hard) {
        const rs = await tx.company.deleteMany({
          where: { uid: { in: uids } },
        });
        return { mode: 'bulk', affected: rs.count };
      } else {
        const rs = await tx.company.updateMany({
          where: { uid: { in: uids } },
  ...
  // Single
  const todo = await prisma.company.findUnique({
    where: { uid: uid! },
    select: { uid: true },
  });
  ...
  return await prisma.$transaction(async (tx) => {
    if (hard) {
      const rs = await tx.company.delete({ where: { uid: uid! } });
      return { mode: 'single', affected: rs ? 1 : 0 };
    } else {
      const rs = await tx.company.update({
        where: { uid: uid! },
        data: { isUse: false, isVisible: false },
      });
      return { mode: 'single', affected: rs ? 1 : 0 };
