...
import ListForm from '@/components/comment/ListForm';
import type { SortBy, SortOrder } from '@/types/comment';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const metadata = getRouteMetadata('comment.index', dictionary, language);
...
export default async function Page({ params, searchParams }: Props) {
  ...
  const dictionary = await getDictionary(language);
  const metadata = getRouteMetadata('comment.index', dictionary, language);
  const url = getRouteUrl('comment.index', language);
  ...
  const baseParams: CommentBaseParams = {
    ...
    sortBy: (one(sp.sortBy) as SortBy) ?? 'createdAt',
