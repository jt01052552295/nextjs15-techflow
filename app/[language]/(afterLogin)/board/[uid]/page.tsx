...
import ShowForm from '@/components/board/ShowForm';
import { showAction } from '@/actions/board/show';
import { listAction as commentsAction } from '@/actions/board/comments';
import {
  HydrationBoundary,
  dehydrate,
  QueryClient,
...
export default async function Page({ params }: Props) {
  ...
  const dictionary = await getDictionary(language);
  const metadata = getRouteMetadata('board.index', dictionary, language);
  const url = getRouteUrl('board.index', language);
  ...
  await qc.prefetchInfiniteQuery({
    queryKey: boardQK.comments(rootBase),
    queryFn: ({ pageParam }) =>
