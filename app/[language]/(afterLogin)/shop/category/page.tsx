...
import ListForm from '@/components/shop/category/ListForm';
import type { SortBy, SortOrder } from '@/types/shop/category';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const metadata = getRouteMetadata('shopCategory.index', dictionary, language);
...
export default async function Page({ params, searchParams }: Props) {
  ...
  const dictionary = await getDictionary(language);
  const metadata = getRouteMetadata('shopCategory.index', dictionary, language);
  const url = getRouteUrl('shopCategory.index', language);
  ...
  const baseParams: ShopCategoryBaseParams = {
