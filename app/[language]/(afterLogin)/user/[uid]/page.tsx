...
import ShowForm from '@/components/user/ShowForm';
import { showAction } from '@/actions/user/show';
import { listAction as commentsAction } from '@/actions/user/comments';
import {
  HydrationBoundary,
  dehydrate,
  QueryClient,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const metadata = getRouteMetadata('user.index', dictionary, language);
...
export default async function Page({ params }: Props) {
  ...
  const dictionary = await getDictionary(language);
  const metadata = getRouteMetadata('user.index', dictionary, language);
  const url = getRouteUrl('user.index', language);
  ...
  await qc.prefetchInfiniteQuery({
    queryKey: userQK.comments(rootBase),
    queryFn: ({ pageParam }) =>
