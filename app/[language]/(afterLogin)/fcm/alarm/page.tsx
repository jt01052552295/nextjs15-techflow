...
import ListForm from '@/components/fcm/alarm/ListForm';
import type { SortBy, SortOrder } from '@/types/fcm/alarm';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const metadata = getRouteMetadata('fcmAlarms.index', dictionary, language);
...
export default async function Page({ params, searchParams }: Props) {
  ...
  const dictionary = await getDictionary(language);
  const metadata = getRouteMetadata('fcmAlarms.index', dictionary, language);
  const url = getRouteUrl('fcmAlarms.index', language);
  ...
  const baseParams: FcmAlarmBaseParams = {
