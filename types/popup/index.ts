...
export type IPopupPart = Partial<IPopup>;

export type IPopupListRow = IPopup & {
  startTime: string; // DTO에서 ISO 문자열로 변환
  endTime: string; // DTO에서 ISO 문자열로 변환
...
export type ListResult<T = IPopup> = {
  items: T[];
  nextCursor?: string;
  totalAll: number;
