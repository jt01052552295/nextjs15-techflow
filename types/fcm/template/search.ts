...
export const DEFAULTS: FcmTemplateBaseParams = {
  sortBy: 'idx',
  order: 'desc',
  limit: 20,
};

const sortBySet = new Set(['idx', 'type']);
...
export function isSameBaseParams(
  a: FcmTemplateBaseParams,
  b: FcmTemplateBaseParams,
) {
  return (
    norm(a.q) === norm(b.q) &&
