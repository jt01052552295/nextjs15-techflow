  ...
  z
    .object({
      ...
      buyMax: z.coerce
        ...
        .min(0, { message: messages.numeric })
        .default(0),
      isUse: z.coerce.boolean().optional().default(true),
      isVisible: z.coerce.boolean().optional().default(true),
      isSoldout: z.coerce.boolean().optional().default(false),
...
export const CreateSchema = (messages: Record<string, any>) =>
  z.object({
    ...
    // 파일은 1개 이상
    // files: z.array(ShopFileDetailsSchema).min(1, {
    //   message: messages.array_min1 || '최소 1개의 이미지를 등록하세요.',
    // }),
...
export const PatchSchema = (msg: any) => CreateSchema(msg).partial();
