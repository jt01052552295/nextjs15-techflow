...
export const listSortAction = async (
  ...
  const current = await prisma.config.findUnique({ where: { uid } });
  ...
  switch (direction) {
    case 'up':
      target = await prisma.config.findFirst({
    ...
    case 'down':
      ...
      break;
    case 'top':
      const max = await prisma.config.aggregate({
        _max: { sortOrder: true },
      });
    ...
    case 'bottom':
      const min = await prisma.config.aggregate({
        _min: { sortOrder: true },
      });
      await prisma.config.update({
        where: { uid },
