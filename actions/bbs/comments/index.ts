...
import { IBBSCommentPart, IBBSCommentRow } from '@/types/comment';
...
import {
  ...
  likeComment,
} from '@/services/bbs.comment.service';
import { fmtDateD } from '@/lib/util';

export async function listAction(
  params: CommentListParams,
): Promise<CommentListResult<IBBSCommentRow>> {
  try {
...
export async function createAction(data: IBBSCommentPart) {
  ...
  const { bdTable, pid, content, parentIdx = null } = parsed.data;
  ...
  try {
    ...
    const full = await prisma.bBSComment.findUnique({
...
export async function updateAction(data: IBBSCommentPart) {
  const language = await ckLocale();
  const dictionary = await getDictionary(language);
  const missingFields = await __ts('common.form.missingFields', {}, language);
  ...
  const { bdTable, uid, content, pid } = parsed.data;
  if (!bdTable || !uid || !pid || !content) {
    return {
      status: 'error',
      message: missingFields,
    };
  ...
  try {
    ...
    // 2) 작성자 포함해서 재조회(액션 레이어에서 수행)
    const updated = await prisma.bBSComment.findUnique({
      where: { uid },
      include: {
        user: {
          select: {
...
export async function deleteCommentAction(data: IBBSCommentRow) {
  const language = await ckLocale();
  const missingFields = await __ts('common.form.missingFields', {}, language);
...
export async function likeAction(payload: any) {
  ...
  const { bdTable, commentId } = payload;

  if (!bdTable || !commentId) {
    return {
      status: 'error',
      message: missingFields,
    };
