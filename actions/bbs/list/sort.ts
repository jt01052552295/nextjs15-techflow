...
export const listSortAction = async (
  ...
  const current = await prisma.bBS.findUnique({ where: { uid } });
  ...
  switch (direction) {
    case 'up':
      target = await prisma.bBS.findFirst({
    ...
    case 'down':
      ...
      break;
    case 'top':
      const max = await prisma.bBS.aggregate({
        _max: { sortOrder: true },
      });
      await prisma.bBS.update({
        where: { uid },
    ...
    case 'bottom':
      const min = await prisma.bBS.aggregate({
        _min: { sortOrder: true },
      });
      await prisma.bBS.update({
        where: { uid },
